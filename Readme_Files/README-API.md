...
To access protected routes, you must first exchange your credentials for a.tsON Web Token (JWT).

### PowerShell Command:
```powershell
$response = Invoke-RestMethod -Uri "http://localhost:3000/api/authenticate" `
  -Method Post `
  -ContentType "application.tson" `
...
3. **Body Tab**: 
   ...
   * Paste: 
     ...
     ```
4. **Send**: Click Send. Copy the `token` string from the.tsON response.
